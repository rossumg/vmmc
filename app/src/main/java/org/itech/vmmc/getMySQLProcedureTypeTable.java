 ...
 */
class getMySQLProcedureTypeTable extends AsyncTask<String, String, String> {
    ...
    getMySQLProcedureTypeTable(Context context, DBHelper dbhelp) {
        this.dbhelp = dbhelp;
        this._context = context;
        this._db = dbhelp.getWritableDatabase();
        this._db.execSQL("delete from procedure_type");
    }

    @Override
    protected String doInBackground(String... args) {
        Log.d(LOG, "getMySQLProcedureTypeTable doInBackground ");
        ...
        Log.d(LOG, "getMySQLProcedureTypeTable username/password: " + username + " " + password);
        String datatable = "getProcedureType";
        try {
            URL url = null;
            try {
                url = new URL(MainActivity.GET_TABLE_URL);
                Log.d(LOG, "getMySQLProcedureTypeTable GET_TABLE_URL " + url.toString());
            ...
            if (success == 1) {
                ...
                for (i = 0; i < _array.length(); i++) {
                    ...
                    Log.d(LOG, "getMySQLProcedureTypeTable:doInBackground: " + _name );
                    String _insert =
                    ...
                    try {
                        ...
                        //Log.d(LOG, "getMySQLFacilitatorTypeTable personInsert " + personInsert.toString() + " " + i);
                        //Log.d(LOG, "getMySQLFacilitatorTypeTable personInsert " + " " + i);

                        _db.execSQL(_insert.toString());

                    } catch (Exception ex) {
                        Log.d(LOG, "getMySQLProcedureTypeTable loop exception > " + ex.toString());
                    }
                } // foreach
            ...
            e.printStackTrace();
            Log.d(LOG, "getMySQLProcedureTypeTable exception > " + e.toString());
            syncAudit.set_status("getMySQLProcedureTypeTable exception:" + e.toString());
        }
        Log.d(LOG, "getMySQLProcedureTypeTable.doInBackground end");
        return Integer.toString(i);
    }

    protected void onPostExecute(String result) {
        Log.d(LOG, "getMySQLProcedureTypeTable:onPostExecute: " + result);
        syncAudit.set_progress("getMySQLProcedureTypeTable:" + result);
