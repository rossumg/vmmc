 ...
 */
class getMySQLAclTable extends AsyncTask<String, String, String> {
    ...
    getMySQLAclTable(Context context, DBHelper dbhelp) {
        this.dbhelp = dbhelp;
        this._context = context;
        this._db = dbhelp.getWritableDatabase();
        this._db.execSQL("delete from acl");
    }

    @Override
    protected String doInBackground(String... args) {
        Log.d(LOG, "getMySQLAclTable doInBackground ");
        ...
        Log.d(LOG, "getMySQLAclTable username/password: " + username + " " + password);
        String datatable = "getAcl";
        try {
            URL url = null;
            try {
                url = new URL(MainActivity.GET_TABLE_URL);
                Log.d(LOG, "getMySQLAclTable GET_TABLE_URL " + url.toString());
            ...
            if (success == 1) {
                ...
                for (i = 0; i < _array.length(); i++) {
                    ...
                    _id = _id.replace("'", "''");
                    String _acl = _rec.getString("acl");
                    _acl = _acl.replace("'", "''");

                    Log.d(LOG, "getMySQLAclTable:doInBackground: " + _acl );
                    String _insert =
                            "insert into acl "
                                    + "(id, acl ) "
                                    + " values(" + "'" + _id + "'" + ", '" + _acl + "'"
                    ...
                    try {
                        ...
                        //Log.d(LOG, "getMySQLAclTable personInsert " + personInsert.toString() + " " + i);
                        //Log.d(LOG, "getMySQLAclTable personInsert " + " " + i);

                        _db.execSQL(_insert.toString());

                    } catch (Exception ex) {
                        Log.d(LOG, "getMySQLAclTable loop exception > " + ex.toString());
                    }
                } // foreach
            ...
            e.printStackTrace();
            Log.d(LOG, "getMySQLAclTable exception > " + e.toString());
            syncAudit.set_status("getMySQLAclTable exception:" + e.toString());
        }
        Log.d(LOG, "getMySQLAclTable.doInBackground end");
        return Integer.toString(i);
    }

    protected void onPostExecute(String result) {
        Log.d(LOG, "getMySQLAclTable:onPostExecute: " + result);
        syncAudit.set_progress("getMySQLAclTable:" + result);
