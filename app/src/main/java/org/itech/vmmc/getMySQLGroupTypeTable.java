 ...
 */
class getMySQLGroupTypeTable extends AsyncTask<String, String, String> {
    ...
    getMySQLGroupTypeTable(Context context, DBHelper dbhelp) {
        this.dbhelp = dbhelp;
        this._context = context;
        this._db = dbhelp.getWritableDatabase();
        this._db.execSQL("delete from group_type");
    }

    @Override
    protected String doInBackground(String... args) {
        Log.d(LOG, "getMySQLGroupTypeTable doInBackground ");
        ...
        Log.d(LOG, "getMySQLGroupTypeTable username/password: " + username + " " + password);
        String datatable = "getGroupType";
        try {
            URL url = null;
            try {
                url = new URL(MainActivity.GET_TABLE_URL);
                Log.d(LOG, "getMySQLGroupTypeTable GET_TABLE_URL " + url.toString());
            ...
            if (success == 1) {
                ...
                for (i = 0; i < _array.length(); i++) {
                    ...
                    Log.d(LOG, "getMySQLGroupTypeTable:doInBackground: " + _name );
                    String _insert =
                    ...
                    try {
                        ...
                        //Log.d(LOG, "getMySQLGroupTypeTable personInsert " + personInsert.toString() + " " + i);
                        //Log.d(LOG, "getMySQLGroupTypeTable personInsert " + " " + i);

                        _db.execSQL(_insert.toString());

                    } catch (Exception ex) {
                        Log.d(LOG, "getMySQLGroupTypeTable loop exception > " + ex.toString());
                    }
                } // foreach
            ...
            e.printStackTrace();
            Log.d(LOG, "getMySQLGroupTypeTable exception > " + e.toString());
            syncAudit.set_status("getMySQLGroupTypeTable exception:" + e.toString());
        }
        Log.d(LOG, "getMySQLGroupTypeTable.doInBackground end");
        return Integer.toString(i);
    }

    protected void onPostExecute(String result) {
        Log.d(LOG, "getMySQLGroupTypeTable:onPostExecute: " + result);
        syncAudit.set_progress("getMySQLGroupTypeTable:" + result);
