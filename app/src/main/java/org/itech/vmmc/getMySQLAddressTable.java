 ...
 */
class getMySQLAddressTable extends AsyncTask<String, String, String> {
    ...
    getMySQLAddressTable(Context context, DBHelper dbhelp) {
        this.dbhelp = dbhelp;
        this._context = context;
        this._db = dbhelp.getWritableDatabase();
        this._db.execSQL("delete from address");
    }

    @Override
    protected String doInBackground(String... args) {
        Log.d(LOG, "getMySQLAddressTable doInBackground ");
        ...
        Log.d(LOG, "getMySQLAddressTable username/password: " + username + " " + password);
        String datatable = "getAddress";
        try {
            URL url = null;
            try {
                url = new URL(MainActivity.GET_TABLE_URL);
                Log.d(LOG, "getMySQLAddressTable GET_TABLE_URL " + url.toString());
            ...
            if (success == 1) {
                ...
                for (i = 0; i < _array.length(); i++) {
                    ...
                    Log.d(LOG, "getMySQLAddressTable:doInBackground: " + _name + ", " + _region_id );
                    String _insert =
                            "insert into address "
                                    + "(id, name, region_id) "
                                    + " values("
                    ...
                    try {
                        ...
                        Log.d(LOG, "getMySQLAddressTable _insert " + _insert.toString() + " " + i);
                        //Log.d(LOG, "getMySQLAddressTable personInsert " + " " + i);

                        _db.execSQL(_insert.toString());

                    } catch (Exception ex) {
                        Log.d(LOG, "getMySQLAddressTable loop exception > " + ex.toString());
                    }
                } // foreach
            ...
            e.printStackTrace();
            Log.d(LOG, "getMySQLAddressTable exception > " + e.toString());
            syncAudit.set_status("getMySQLAddressTable exception:" + e.toString());
        }
        return Integer.toString(i);
    }

    protected void onPostExecute(String result) {
        Log.d(LOG, "getMySQLAddressTable:onPostExecute: " + result);
        syncAudit.set_progress("getMySQLAddressTable:" + result);
